    ...
    try {
      if (fs.existsSync(this.filePath)) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          this.data.password_resets = Array.isArray(parsed.password_resets) ? parsed.password_resets : [];
...
// Live Status Section Visibility Settings
function getShowStatusSection(db = getDB()) {
  return db.data.settings.show_status_section !== '0';
}

function setShowStatusSection(enabled, db = getDB()) {
  db.data.settings.show_status_section = enabled ? '1' : '0';
  db.save();
  return enabled;
}
...
function getAnalyticsSettings(db = getDB()) {
  const s = db.data.settings;
  return {
    gaMeasurementId: s.ga_measurement_id || '',
    gaEnabled: s.ga_enabled === '1'
  };
}

function saveAnalyticsSettings(settings, db = getDB()) {
  if (settings.gaMeasurementId !== undefined) {
    db.data.settings.ga_measurement_id = settings.gaMeasurementId.trim();
  }
  if (settings.gaEnabled !== undefined) {
    db.data.settings.ga_enabled = (settings.gaEnabled === true || settings.gaEnabled === '1' || settings.gaEnabled === 1) ? '1' : '0';
  }
...
function changeUserPassword(userId, currentPassword, newPassword, db = getDB()) {
...
function createPasswordResetToken(emailOrUsername, db = getDB()) {
  ...
  if (!Array.isArray(db.data.password_resets)) {
    db.data.password_resets = [];
  }

  // Remove existing reset tokens for this user
  db.data.password_resets = db.data.password_resets.filter(
...
function resetPasswordWithToken(token, newPassword, db = getDB()) {
  if (!token) {
  ...
  const resetRecord = db.data.password_resets.find(r => r.token === token);
  if (!resetRecord) {
    throw new Error('Password reset link is invalid or has expired.');
  }

  if (new Date(resetRecord.expires_at) < new Date()) {
    db.data.password_resets = db.data.password_resets.filter(r => r.token !== token);
    db.save();
    throw new Error('Password reset link has expired. Please request a new one.');
  }

  const user = db.data.users.find(u => Number(u.id) === Number(resetRecord.user_id));
  if (!user) {
    throw new Error('User not found.');
  }
  ...
  db.data.password_resets = db.data.password_resets.filter(r => Number(r.user_id) !== Number(user.id));

  // Invalidate previous sessions
  db.data.sessions = db.data.sessions.filter(s => Number(s.user_id) !== Number(user.id));
